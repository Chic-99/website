...
as $$
  select a.reference_number, a.status, a.doctor_name, a.specialty,
         a.preferred_date, a.preferred_time, a.patient_name, a.patient_phone,
         a.patient_age, a.patient_gender, a.patient_type, a.contact_preference,
         a.created_at
...
-- =============================================================================
-- 5b. PUBLIC APPOINTMENT CREATION (security definer RPC)
-- The public INSERT policy above allows anon to insert, but a plain
-- `insert ... returning *` (or supabase-js `.insert().select()`) also
-- requires SELECT access under RLS to read the row back — which anon
-- deliberately does not have. This function inserts AND returns the new
-- row's details in one trusted step, without granting broad table access.
-- =============================================================================
...
begin
  insert into public.appointments (
    doctor_id, doctor_name, specialty, preferred_date, preferred_time,
    patient_name, patient_phone, patient_email, patient_age, patient_gender,
    contact_preference, patient_type
  ) values (
    p_doctor_id, p_doctor_name, p_specialty, p_preferred_date, p_preferred_time,
    p_patient_name, p_patient_phone, p_patient_email, p_patient_age, p_patient_gender,
    p_contact_preference, p_patient_type
  )
  returning appointments.id into v_id;

  return query
  select a.id, a.reference_number, a.status, a.doctor_name, a.specialty, a.preferred_date, a.preferred_time, a.created_at
  from public.appointments a
  where a.id = v_id;
end;
$$;
...
grant execute on function public.create_appointment(
  uuid, text, text, date, text, text, text, integer, text, text, text, text
) to anon, authenticated;
