...
function emptyState(){
  return { doctorId:'', doctorName:'', specialty:'', date:'', time:'', name:'', phone:'', age:'', gender:'', contactPref:'Phone Call', patientType:'New Patient' };
}
...
async function submitAppointment(){
  ...
  const payload = {
    doctor_id: bookingState.doctorId || null,
    doctor_name: bookingState.doctorName,
    specialty: bookingState.specialty,
    preferred_date: bookingState.date,
    preferred_time: bookingState.time,
    patient_name: bookingState.name,
    patient_phone: bookingState.phone,
    patient_age: bookingState.age ? parseInt(bookingState.age, 10) : null,
    patient_gender: bookingState.gender || null,
...
function renderConfirmation(record, emailFailed){
  ...
  document.getElementById('printConfirm').addEventListener('click', () => printAppointmentSlip({ ref: record.reference_number, ...record }));
}
